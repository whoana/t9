...
public class ChannelManager {
	 ...
	 * @throws Exception
	 */
	private void startRestChannel(ChannelConfig config, ChannelExceptionHandler channelExceptionHandler,
		...
		restChannel = new RestChannel(name, config.getMaxCacheSize(), config.getDelayForMaxCache(), selectCaches,
				taskExecutor);
		if (tpm != null)
			restChannel.setThroughputMonitor(tpm);
	 ...
	 * @throws Exception
	 */
	private void startMOMChannel(ChannelConfig config, ChannelExceptionHandler channelExceptionHandler,
			OldStateCheckHandlerConfig oschc) throws Exception {
		String name = config.getName();
	...
	/**
	 * @deprecated 20200508 This funciton was moved to the DatabasePolicyHandler's
	 *             function.
	 */
