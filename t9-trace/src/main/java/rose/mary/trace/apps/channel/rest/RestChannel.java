 ...
 */
package rose.mary.trace.apps.channel.rest;
...
import rose.mary.trace.apps.handler.OldStateCheckHandler;
import rose.mary.trace.apps.manager.config.OldStateCheckHandlerConfig;
...
public class RestChannel extends Channel{
	...
	public RestChannel(String name,	int maxCacheSize, 
			long delayForMaxCache, List<CacheProxy<String, Trace>> caches, ThreadPoolTaskExecutor taskExecutor, OldStateCheckHandlerConfig oschc) {
		super(name, false, 0, 0, 0, 0, maxCacheSize, delayForMaxCache, null);
		this.caches = caches; 
		this.sch = new OldStateCheckHandler(oschc);
	...
	public void start() throws Exception {
		isShutdown = false;
		logger.info(Util.join("start channel:" , this.getClass().getName() , "[" + Thread.currentThread().getName() + "]"));		
	}
	
	@Override
	public void stop() {
		isShutdown = true;
		logger.info(Util.join("stop channel:" , this.getClass().getName() , "[" + Thread.currentThread().getName() + "]"));
	}
	...
	public void consumeAsync(List<Map<String, Object>> traces) throws Exception{
		if(isShutdown) throw new Exception("RestChannel(" + this.hashCode() + ") channel is not open yet.");		
