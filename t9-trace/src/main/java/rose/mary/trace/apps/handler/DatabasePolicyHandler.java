...
//@Aspect
//@Component
public class DatabasePolicyHandler implements Runnable{
	...
	public void start() throws Exception {
		...
		thread.setName("DatabasePolicyHandler");
		isShutdown = false;
		thread.start();
	}
...
//	}
	...
	public void runAsap() {
		logger.info(Util.join("start DatabasePolicyHandler:[" + name + "]"));
		while (true) {
			...
			try {
				...
				if(systemService.databaseHealthCheck()) {
					...
					if(channels != null && channels.size() > 0) {
						for(Channel channel : channels) {
							logger.info(Util.join("channel:", channel.getName(), " isHealthCheck:", channel.isHealthCheck(), ", running:", !channel.isShutdown()));
							if(channel.isHealthCheck() && !channel.isShutdown()) {
								...
								if(!channel.ping()) {
									...
									try {
										...
										logger.error(Util.join("I can not restart channel[", channel.getName() , "]"), e);
									}
								}else {
									logger.info(Util.join("channel[", channel.getName() , "] health check result: true"));
								}
				...
				//에러 캐시에 뭔가 쌓여 있으면.
//				boolean hasDatabaseProblem = errorCache1.size() > 0 || errorCache2.size() > 0 || !databaseHealth;
...
//					}
					...
					serverManager.addWarnning("데이터베이스와의 접속 및 서비스에 문제가 없다면 처리하지 못한 트레킹 및 트레킹명세 메시지는 리커버리 모드 수행을 통해 복구를 진행해 주세요.");
				...
				logger.error("DatabasePolicyHandler exception:", t);
				try {
		...
		isShutdown = true;
		logger.info(Util.join("stop DatabasePolicyHandler:[" + name + "]"));
