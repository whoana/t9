...
public class ApplicationConfig {
	...
	public MonitorManager monitorManager(
			...
			@Autowired @Qualifier("tpm4") ThroughputMonitor tpm4
		) throws Exception {
	...
	@Bean(initMethod = "prepare")
	public InterfaceCacheManager interfaceCacheManager(@Autowired ConfigurationManager configurationManager, @Autowired ThreadPoolTaskScheduler taskScheduler, @Autowired InterfaceService service, @Autowired CacheManager cacheManager) throws Exception {
		InterfaceCacheManager manager = new InterfaceCacheManager(configurationManager.getInterfaceCacheManagerConfig(), taskScheduler, service,  cacheManager);
		return manager;
	}
	
	
	@Bean
	public ChannelManager channelManager(@Autowired ConfigurationManager configurationManager, @Autowired CacheManager cacheManager, @Autowired @Qualifier("tpm1") ThroughputMonitor tpm1) throws Exception {		
		return new ChannelManager(configurationManager.getChannelManagerConfig(), cacheManager, tpm1);
	}
	
	
	@Bean
	public LoaderManager loaderManager(@Autowired ConfigurationManager configurationManager, @Autowired TraceService traceService,  @Autowired CacheManager cacheManager, @Autowired @Qualifier("tpm2") ThroughputMonitor tpm2) throws Exception {		
		return new LoaderManager(configurationManager.getLoaderManagerConfig(), traceService, cacheManager, tpm2);
	}

	
	@Bean
	public BoterManager boterManager(@Autowired ConfigurationManager configurationManager,  @Autowired CacheManager cacheManager, @Autowired @Qualifier("tpm3") ThroughputMonitor tpm3) throws Exception {		
		return new BoterManager(configurationManager.getBoterManagerConfig(), cacheManager, tpm3);
	}
	
	@Bean
	public BotLoaderManager botLoaderManager(@Autowired ConfigurationManager configurationManager,  @Autowired BotService botService, @Autowired CacheManager cacheManager, @Autowired @Qualifier("tpm4") ThroughputMonitor tpm4) throws Exception {		
		return new BotLoaderManager(configurationManager.getBotLoaderManagerConfig(), botService, cacheManager, tpm4);
	}
	
	@Bean
	public FinisherManager finisherManager(@Autowired ConfigurationManager configurationManager,  @Autowired CacheManager cacheManager) throws Exception {		
		return new FinisherManager(configurationManager.getFinisherManagerConfig(), cacheManager);
	}
	
	@Bean
	public TraceErrorHandlerManager traceErrorHandlerManager(@Autowired ConfigurationManager configurationManager,  @Autowired CacheManager cacheManager, @Autowired TraceService traceService, @Autowired MessageSource messageSource) throws Exception {		
		return new TraceErrorHandlerManager(configurationManager.getTraceErrorHandlerManagerConfig(), cacheManager, traceService, messageSource);
	}
	
	@Bean
	public BotErrorHandlerManager botErrorHandlerManager(@Autowired ConfigurationManager configurationManager,  @Autowired CacheManager cacheManager, @Autowired BotService botService, @Autowired MessageSource messageSource) throws Exception {		
		return new BotErrorHandlerManager(configurationManager.getBotErrorHandlerManagerConfig(), cacheManager, botService, messageSource);
	}

	@Bean
	public UnmatchHandlerManager unmatchHandlerManager(@Autowired ConfigurationManager configurationManager,  @Autowired CacheManager cacheManager, @Autowired BotService botService, @Autowired MessageSource messageSource) throws Exception {		
		return new UnmatchHandlerManager(configurationManager.getUnmatchHandlerManagerConfig(), cacheManager, botService, messageSource);
	}
	
	@Bean
	public SystemErrorTestManager systemErrorTesterManager(@Autowired ConfigurationManager configurationManager,  @Autowired SystemService systemService) throws Exception {		
	...
	public DatabasePolicyHandlerManager databasePolicyHandlerManager(@Autowired ConfigurationManager configurationManager,  @Autowired CacheManager cacheManager, @Autowired ServerManager serverManager, @Autowired ChannelManager channelManager, @Autowired SystemService systemService, @Autowired MessageSource messageSource) throws Exception {		
		return new DatabasePolicyHandlerManager(configurationManager.getDatabasePolicyConfig(), systemService, serverManager, channelManager, messageSource, cacheManager);
	}
