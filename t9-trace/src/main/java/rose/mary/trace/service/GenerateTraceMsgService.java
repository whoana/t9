...
public class GenerateTraceMsgService {
	...
	public String generate(Map<String, String> params) throws Throwable {
		...
		try {
			...
			TraceMsgGenerator tmg = new TraceMsgGenerator(hostname, port, qmgrName, userId, password, channelName,
					queueName, module, generateCount, commitCount, data, tmc, cacheManager.getInterfaceCache());
			...
			sw.start();
			tmg.generate(params.get("integrationId"));
			sw.stop();
			logger.info("------------------------------------------");
	...
	public static boolean checkParams(Map<String, String> params) {
		...
		if (ok) {
			try {
				Integer.parseInt((String) params.get("port"));
			} catch (NumberFormatException e) {
				ok = false;
				System.out.println("-po 파라미터는 숫자여야 합니다.");
			}
			try {
				Integer.parseInt((String) params.get("generateCount"));
			} catch (NumberFormatException e) {
				ok = false;
				System.out.println("-ge 파라미터는 숫자여야 합니다.");
			}
			try {
				Integer.parseInt((String) params.get("commitCount"));
			} catch (NumberFormatException e) {
				ok = false;
				System.out.println("-co 파라미터는 숫자여야 합니다.");
			}
		}
