	...
	public static void main(String[] args) {
		...
		try {
			...
			process = Runtime.getRuntime()
					.exec("/Library/Java/JavaVirtualMachines/jdk1.8.0_251.jdk/Contents/Home/bin/java", envs);
		 ...
		 * "/Users/whoana/DEV/workspace-tracking/projectq-support/a");
		 * //Path path = FileSystems.getDefault().getPath(".");
		 * Iterator<Path> paths = path.iterator();
		 * Path checkPath = FileSystems.getDefault().getPath("");
		 * for (Path path2 : path) {
		 * checkPath = FileSystems.getDefault().getPath(checkPath.toString() +
		 * File.separator + path2.toString());
		 * 
		 * System.out.println("checkPath:" + checkPath.toString() + ", exist:" +
		 * Files.exists(checkPath) + ", isWritable:" + Files.isWritable(checkPath));
		 * }
